...
class _LudoAppState extends State<LudoApp> {
  ...
  void _gameListener() {
    ...
    if (chat != null && chat.message.isNotEmpty && chat.timestamp != lastChatTimestamp) {
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        if (_controller.game?.status == 'finished') {
          bool iWon = _controller.user != null ? _controller.game!.winnerUid == _controller.user!.uid : false;
          String winnerColor = _controller.getPlayerIndex(_controller.game!.winnerUid) == 0 ? "BLUE" : "RED";
          return EndGame(
            iWon: iWon,
            winnerName: _controller.getPlayerDisplayTitle(_controller.game!.winnerUid),
                    ...
                    child: Column(
                      children: [
                        Row(
                          ...
                          children: [
                            Text(
                                _controller.game?.isTestModeActive == true ? "🎲 Sandbox Mode" : "🎲 Ludo Battle",
                        ...
                        if (_controller.game?.status == 'waiting')
                          Container(
